...
_Last updated: 2026-04-17 (session 20 — storm data import ✅ 3,257 counties 10yr uploaded to prod; wildfire/CAL FIRE ArcGIS endpoints down — need working URLs; prod deployed commit 03d889c)_
...
- [ ] **Import USFS Wildfire Risk data** — Script ready: `wildfire-import.js`. ⚠️ USFS ArcGIS endpoint (`apps.fs.usda.gov/arcx`) times out — server unreachable. Need alternative: FEMA NRI flat file download or alternative ArcGIS Online service. `/v1/risk` auto-activates `data_coverage.wildfire=true` when DB present.
