...
- ⏳ **Climate Risk Score per address** IN PROGRESS (session 24) — extending `/v1/risk` with heat, drought, sea level rise, precipitation, landslide dimensions. All free federal sources (NASA NEX-GDDP-CMIP6, NOAA Atlas 14, USGS landslide, NOAA SLR). Multi-dimensional response: `climate_risk.composite` + per-hazard scores. Earthquake + drought dimensions wired (`getEarthquakeRisk` + `getDroughtRisk` in `geocode.js`); composite score weighted avg of flood/wildfire/storm/earthquake/drought.
