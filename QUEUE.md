...
_Last updated: 2026-04-17 (session 19 — Phase 2: docs tagline ✅, vs-SmartyStreets ✅, Builder enrichment 500/mo ✅, Pro Compliance tier ✅, SLA doc ✅, activation funnel 80% warn ✅, Day 1 curl quickstart ✅)_
...
- [ ] **Import USFS Wildfire Risk data** — Script ready: `wildfire-import.js`. **Run on staging:** `node wildfire-import.js --db=/data/risk.db --limit=100` (test), then full run. Verify: `sqlite3 /data/risk.db "SELECT COUNT(*) FROM wildfire_risk"`. Upload risk.db via upload-chunk, serve from `/data/risk.db`. `/v1/risk` auto-activates `data_coverage.wildfire=true` when DB present.
- [ ] **Import NOAA severe weather history** — Script ready: `storm-import.js`. **Run on staging:** `node storm-import.js --db=/data/risk.db --years=10`. Downloads 10yr NOAA Storm Events CSV per year, aggregates by county FIPS. Verify count. Upload via upload-chunk (append to same risk.db).
- [ ] **Import CAL FIRE FHSZ (Fire Hazard Severity Zones)** — Script ready: `calfire-import.js`. **Run on staging:** `node calfire-import.js --db=/data/risk.db --limit=500` (test), then full run. Grid-based lat/lon lookup (0.001° cells). CA addresses only. Verify count. Upload via upload-chunk (append to same risk.db).
