...
| **Monorepo structure** | `src/api/v1/`, `src/api/v2/`, `src/db/`, `src/dashboard/`, `src/web/`, `scripts/` | Pipeline scripts moved from root; placeholder index.ts per module; web-server.js worker paths updated |
| **render.yaml skeleton** | `render.yaml` | Three service stubs: geoclear-api (plan: pro), geoclear-marketing (Astro), geoclear-pipeline (worker); fromSecret refs for XATA_DATABASE_URL, NAD_ADMIN_SECRET, STRIPE_* |
| **Drizzle + pg.Pool skeleton** | `src/db/client.ts`, `src/db/schema.ts`, `drizzle.config.ts` | pg.Pool singleton; schema.ts placeholder (populated by drizzle-kit introspect after Q-161); drizzle.config.ts → XATA_DATABASE_URL |
| **GitHub Actions CI/CD** | `.github/workflows/db-backup.yml`, `.github/workflows/data-quality.yml` | Weekly Xata schema snapshot (Sunday 03:00 UTC); Soda Core data quality scan (triggered after backup) |
| **Migration roadmap** | `docs/roadmap.md` | Phase 0 (xata-clone + schema capture), Phase 1 (Hono + pg.Pool), Phase 2 (PostGIS + Soda Core); zero lock-in exit policy |
