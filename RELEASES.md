...
- **MCP outcome-based x402 pricing (Phase 3)** — 4 new premium MCP tools with per-tool USDC pricing on Base: `climate_risk_decision` ($0.02, replaces $2-4 First Street/ZestyAI), `hmda_compliance_check` ($0.05, replaces $3-15 flood determination), `drone_deliverability` ($0.05, replaces human dispatch review), `full_risk_assessment` ($0.10, all risk dimensions). Base tools (verify_address etc.) reduced to $0.002. `_buildMcpPaymentRequirements(toolName)` uses `TOOL_PRICES` map; `mcpAuth` extracts tool name from `req.body.params.name` for per-call pricing. Premium tools gated from free-tier sessions. `_mcpRiskLookup()` helper runs full FEMA/wildfire/storm/earthquake/drought/FAA/census pipeline.
