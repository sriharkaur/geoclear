...
Agent(
  ...
  description="PRISM-10 Performance review",
  prompt="[Full Performance review prompt with project context, all relevant files read, and the complete content of this skill file.]"
)
```
Write the sub-agent output to `architecture/ARCH-DEV_ARCH_PERFORMANCE-{{YYYY-MM-DD}}.md`, log any decisions to DECISIONS.md, then continue.
