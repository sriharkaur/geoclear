...
│
├── planning/                     ← planning artifacts (from /dev-plan, /bds-bootstrap)
│   ├── EPICS.md                  ← EPIC-{YYYY}-{NNNN} — epic inventory
│   └── PLANNING-INDEX.md         ← index of all epics and features
│
├── strategy/                     ← strategy analyses + goal files
│   ├── GOALS-MARKETING.md        ← MG-{YYYY}-{NNNN} — marketing goals
│   ├── GOALS-DEV.md              ← DG-{YYYY}-{NNNN} — engineering goals
│   ├── GOALS-CUSTOMER.md         ← CGG-{YYYY}-{NNNN} — customer growth goals
│   ├── KPIS.md                   ← KPI-{YYYY}-{NNNN} — KPI registry
│   ├── BUSINESS-IDEAS.md         ← BI-{YYYY}-{NNNN} — business idea archive
│   └── STRATEGY-INDEX.md         ← index of all strategy analyses
│
├── .bds/                         ← BDS operational backbone
│   ├── bds.config.yaml           ← project config (COMMITTED to git)
│   └── bds.db                    ← operational SQLite DB (GITIGNORED)
│
...
- `planning/EPICS.md` — epic inventory (created by /bds-bootstrap Phase 2.5)
- `planning/PLANNING-INDEX.md` — planning artifacts index
...
.bds/*.db-wal
.bds/*.db-shm
# Note: .bds/bds.config.yaml IS committed — it is the framework contract
